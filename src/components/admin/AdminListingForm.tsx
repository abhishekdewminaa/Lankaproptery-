...
export default function AdminListingForm({ user, initialData, onBack, onRefresh, onSuccess }: AdminListingFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    package: initialData?.package || initialData?.package_tier || 'Elite Pro',
    ...
    phone_1: initialData?.phone_1 || initialData?.mobile || '077 123 4567',
    phone_2: initialData?.phone_2 || initialData?.landline || '',
    map_link: initialData?.map_link || initialData?.google_maps_link || '',
  ...
  const saveProperty = useCallback(async (isAutoSave = false) => {
    ...
    try {
      ...
      const payload = {
        ...
        package_tier: formData.package,
        mobile: formData.phone_1,
        landline: formData.phone_2,
        google_maps_link: formData.map_link,
